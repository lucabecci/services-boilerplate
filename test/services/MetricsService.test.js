...
describe("MetricsService", () => {
    ...
    it("MetricsService.server_metrics - correct", async function() {
        ...
        assert.deepEqual(!response.data.message ? true:false, true, "No se recibe lo esperado")
    });
    it("MetricsService.server_metrics - bad_credentials", async function() {
        ...
        try{
            ...
            assert.deepEqual(e.response.data.message, "Invalid credentials", "No se protege de credenciales invalidas")
        }
    });
    it("MetricsService.server_metrics - no_credentials", async function() {
        ...
        try{
            ...
            assert.deepEqual(e.response.data.message, "Send credentials for the access", "No se contempla la falta de credenciales")
        }
    });
    after(function(done){
        server.close()
